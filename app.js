...
function loadDB(){
  return JSON.parse(localStorage.getItem(KEY) || JSON.stringify({ clients: [], visits: [] }));
}
function saveDB(db){
...
function clientTotals(db, clientId){
  ...
  const last = vs.length ? vs.slice().sort((a,b)=> (b.date||"").localeCompare(a.date||""))[0].date : "";
...
function renderClients(){
  ...
  rows.forEach(({c,t,tags})=>{
    ...
    tr.innerHTML = `
      ...
      <td>
        <div class="aBtns">
          <button class="aBtn" data-open="${c.id}">Abrir</button>
          <button class="aBtn" data-del="${c.id}">Borrar</button>
        </div>
      </td>
    `;
    body.appendChild(tr);
...
function renderVisits(){
  ...
  vs.forEach(v=>{
    ...
    tr.innerHTML = `
      ...
      <td>
        <div class="aBtns">
          <button class="aBtn" data-edit="${v.id}">Editar</button>
          <button class="aBtn" data-delv="${v.id}">Borrar</button>
        </div>
      </td>
    `;
    body.appendChild(tr);
...
function saveVisit(){
  ...
  const amount = Number($("vAmount").value||0);
...
async function importJSON(file){
  try{
    ...
    saveDB({ clients: db.clients, visits: db.visits });
    refreshAll();
    alert("Importado ✅");
  }catch{
